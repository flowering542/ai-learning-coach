...
function generateQuestion(qqId: string, student: Student): string {
  ...
  const categoryText = categoryMap[question.category || 'basics'];

  // 显示统计（如果有答题记录）
  let stats = "";
  if (student.totalQuestions > 0) {
    const acc = Math.round((student.correctAnswers / student.totalQuestions) * 100);
    stats = `📊 已答${student.totalQuestions}题，正确率${acc}% | ${categoryText} | ${diffText}\n\n`;
  } else {
    stats = `📊 ${categoryText} | ${diffText}\n\n`;
  }
...
function selectSmartQuestion(student: Student): typeof questionBank[0] {
  ...
  history.forEach(h => {
    // 从题库中找到对应题目的category
    const q = questionBank.find(qb => qb.id === h.questionId);
    if (q && q.category) {
  ...
  // 1. 优先从最少类别中选未做过的题（均衡+新知）
  if (minCategory) {
    const candidates = questionBank.filter(q =>
      q.category === minCategory &&
